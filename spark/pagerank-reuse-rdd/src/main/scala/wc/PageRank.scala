...
object PageRank {
  ...
  def main(args: Array[String]) {
    ...
    val graphRDD = spark.sparkContext.parallelize(1 to  pages,20)
    ...
    println("K^2 pages: "+pages)
    println("No. of Iterations: "+iters)
    println("Sum of PageRanks: "+pageRanks.map(f => f._2).sum())
    println("GraphRDD no. of partitions: "+partitions)
  }
}
